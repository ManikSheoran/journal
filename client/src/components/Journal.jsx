...
function Journal() {
  ...
  const formatted_date = `${date.slice(0, 2)} ${months[parseInt(date.slice(2, 4)) - 1]}, ${date.slice(4)}`;
  ...
  if (error) {
    return (
      <div className="w-full max-w-lg mx-auto my-10 text-center text-red-700 font-semibold bg-red-100 border border-red-300 p-6 rounded-lg shadow-md">
        <span role="img" aria-label="error" className="mr-2">️⚠️</span> {error}
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="w-screen min-h-screen bg-[#FFE3BB] flex flex-col items-center justify-center text-xl text-gray-500">
        <span role="img" aria-label="loading" className="animate-spin inline-block mr-3 text-2xl">⏳</span>
    ...
    return (
      <div className="w-screen min-h-screen bg-[#FFE3BB] flex flex-col items-center justify-center py-12 px-4">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-12 text-center">
          <h2 className="text-2xl font-semibold text-gray-700"><span role="img" aria-label="calendar" className="mr-2 text-3xl">📅</span>No date selected. Please pick a date from the calendar.</h2>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-screen min-h-screen bg-[#FFE3BB] flex flex-col items-center justify-start py-10 px-4">
      <h2 className="text-4xl font-bold text-[#03A6A1] mb-10 text-center">
        <span role="img" aria-label="journal" className="mr-2">📝</span> Journal Entry for {formatted_date}
      </h2>
      ...
      <div className="flex w-full max-w-6xl mx-auto">
        <div className="w-2/3 pr-4">
          {!isEditing ? (
            journal ? (
              <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 sm:p-8 h-full flex flex-col space-y-6">
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <h3 className="font-semibold text-gray-700 text-lg mb-2"><span role="img" aria-label="text" className="mr-2">✍️</span> My Day</h3>
                  <p className="text-gray-600 whitespace-pre-wrap">{journal.content}</p>
                </div>
                <div className="flex space-x-4">
                  <div className="w-1/4">
                    <div className="p-4 bg-gray-50 rounded-lg border border-gray-200 h-full">
                      <h3 className="font-semibold text-gray-700 text-lg mb-2"><span role="img" aria-label="mood" className="mr-2">♥️</span> Mood</h3>
                      <p className="text-gray-600 text-2xl">{moodEmojis.find(m => m.value === journal.mood)?.emoji || journal.mood}</p>
                    </div>
                  </div>
                  <div className="w-3/4">
                    <div className="p-4 bg-gray-50 rounded-lg border border-gray-200 h-full">
                      <h3 className="font-semibold text-gray-700 text-lg mb-2"><span role="img" aria-label="checklist" className="mr-2">✅</span> Todos</h3>
                      {journal.todos.length > 0 ? (
                        <ul className="list-disc list-inside text-gray-600 space-y-1">
                          {journal.todos.map((todo, index) => <li key={index}>{todo}</li>)}
                        </ul>
                      ) : (
                        <p className="text-gray-500 italic">No todos for today.</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 h-full flex flex-col items-center justify-center text-center space-y-6">
                <span role="img" aria-label="empty page" className="text-6xl">🌬️</span>
                <p className="text-xl text-gray-600">Looks like this page is empty.</p>
                <p className="text-gray-500">Ready to jot down some thoughts for {formatted_date}?</p>
                <button
                  onClick={() => setIsEditing(true)}
                ...
                >
                  <span role="img" aria-label="plus" className="mr-2">➕</span> Create Entry
                </button>
              </div>
            )
          ) : (
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 sm:p-8 h-full flex flex-col space-y-6">
              <div>
                <label htmlFor="content" className="block text-md text-gray-600 font-medium mb-1.5"><span role="img" aria-label="text" className="mr-1">✍️</span> What{"'"}s on your mind?</label>
              <textarea
                name="content"
                id="content"
                value={form.content}
              ...
              <div>
                <label htmlFor="mood" className="block text-md text-gray-600 font-medium mb-1.5"><span role="img" aria-label="mood" className="mr-1">♥️</span> How are you feeling?</label>
                <div className="flex space-x-2 sm:space-x-3 justify-around p-2 bg-gray-50 rounded-lg border border-gray-200">
                  {moodEmojis.map(({ emoji, value, label }) => (
                    <button
                      ...
                      className={`text-3xl sm:text-4xl p-2 rounded-lg transition-all duration-200 ease-in-out transform hover:scale-125 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#03A6A1] ${
                        form.mood === value ? 'bg-[#03A6A1] bg-opacity-20 scale-110 ring-2 ring-[#03A6A1]' : 'hover:bg-gray-200'
                      }`}
                    >
                      {emoji}
                    </button>
              ...
              <div>
                <label className="block text-md text-gray-600 font-medium mb-1.5"><span role="img" aria-label="checklist" className="mr-1">✅</span> Any tasks for today?</label>
                <ul className="space-y-3">
                  {form.todos.map((todo, idx) => (
                    <li key={idx} className="flex items-center space-x-3 p-2 bg-gray-50 rounded-md border border-gray-200">
                      <input
                        type="text"
                        value={todo}
                      ...
                      <button
                        onClick={() => {
                          const newTodos = form.todos.filter((_, i) => i !== idx);
                          setForm({ ...form, todos: newTodos });
                        }}
                ...
                >
                  <span role="img" aria-label="plus" className="mr-1">➕</span> Add Todo
                </button>
              </div>
              <div className="flex justify-end space-x-4 mt-auto">
                ...
                >
                  <span role="img" aria-label="cancel" className="mr-2">❌</span> Cancel
                </button>
                <button
                  onClick={handleSave}
                ...
                >
                  <span role="img" aria-label="save" className="mr-2">💾</span> Save
                </button>
              </div>
            </div>
          )}
